...
scp -o StrictHostKeyChecking=no auto_deploy.sh %VPS_USER%@%VPS_IP%:/tmp/

echo 🚀 Executing deployment on VPS...
ssh -o StrictHostKeyChecking=no %VPS_USER%@%VPS_IP% "chmod +x /tmp/auto_deploy.sh && /tmp/auto_deploy.sh"
...
echo   Check logs: ssh %VPS_USER%@%VPS_IP% "journalctl -u cambridge-exam -f"
echo.
pause